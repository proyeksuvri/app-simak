import { useState, useEffect } from 'react'
import { Modal }                from '../ui/Modal'
import { Button }               from '../ui/Button'
import { useWorkUnits }         from '../../hooks/useWorkUnits'
import { usePeriods }           from '../../hooks/usePeriods'
import { useMutateTransaction } from '../../hooks/useMutateTransaction'
import { useBankAccounts }      from '../../hooks/useBankAccounts'
import { useAppContext }         from '../../context/AppContext'
import type { Transaction } from '../../types'

// ── Kategori per jenis ─────────────────────────────────────────────────────────

const KATEGORI_BLU     = ['Biaya Operasional BLU'] as const
const KATEGORI_NON_BLU = ['Pengembalian UKT', 'Admin Bank', 'Pajak Bank'] as const

type JenisPengeluaran = 'BLU' | 'Non BLU'

const JENIS_OPTIONS: { value: JenisPengeluaran; label: string }[] = [
  { value: 'BLU',     label: 'Pengeluaran BLU' },
  { value: 'Non BLU', label: 'Pengeluaran Non BLU' },
]

function getKategoriList(jenis: JenisPengeluaran | '') {
  if (jenis === 'BLU')     return [...KATEGORI_BLU]
  if (jenis === 'Non BLU') return [...KATEGORI_NON_BLU]
  return []
}

// ── Helpers ────────────────────────────────────────────────────────────────────

function formatRupiah(value: string): string {
  const digits = value.replace(/\D/g, '')
  if (!digits) return ''
  return Number(digits).toLocaleString('id-ID')
}

function parseRupiah(formatted: string): number {
  return Number(formatted.replace(/\D/g, '')) || 0
}

// ── Form state ─────────────────────────────────────────────────────────────────

interface FormState {
  transaction_date: string
  no_bukti:         string
  description:      string
  jenis:            JenisPengeluaran | ''
  kode_rekening:    string
  amountFormatted:  string
  bank_account_id:  string
  work_unit_id:     string
}

const emptyForm = (): FormState => ({
  transaction_date: new Date().toISOString().slice(0, 10),
  no_bukti:         '',
  description:      '',
  jenis:            '',
  kode_rekening:    '',
  amountFormatted:  '',
  bank_account_id:  '',
  work_unit_id:     '',
})

// ── Props ──────────────────────────────────────────────────────────────────────

interface BPNKeluarFormModalProps {
  open:      boolean
  onClose:   () => void
  onSuccess: () => void
  editTx?:   Transaction
}

// ── Component ──────────────────────────────────────────────────────────────────

export function BPNKeluarFormModal({ open, onClose, onSuccess, editTx }: BPNKeluarFormModalProps) {
  const { currentUser } = useAppContext()
  const { workUnits }   = useWorkUnits()
  const { activePeriod } = usePeriods()
  const { insertTransaction, updateTransaction, saving, error } = useMutateTransaction()
  const { accounts: bankAccounts } = useBankAccounts(true)

  const lockedUnitId = currentUser?.role === 'bendahara_pembantu' ? (currentUser.unitId ?? null) : null
  const isEdit = !!editTx

  const [form, setForm]         = useState<FormState>(emptyForm)
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<keyof FormState, string>>>({})

  // Reset / pre-fill form setiap kali modal dibuka
  useEffect(() => {
    if (!open) return
    if (editTx) {
      // Deteksi jenis dari kategori yang tersimpan
      const storedKategori = editTx.kategori as string
      const jenis: JenisPengeluaran | '' = KATEGORI_BLU.includes(storedKategori as any)
        ? 'BLU'
        : KATEGORI_NON_BLU.includes(storedKategori as any)
          ? 'Non BLU'
          : ''
      setForm({
        transaction_date: editTx.tanggal,
        no_bukti:         editTx.nomorBukti === '-' ? '' : editTx.nomorBukti,
        description:      editTx.deskripsi,
        jenis,
        kode_rekening:    storedKategori,
        amountFormatted:  editTx.nominal.toLocaleString('id-ID'),
        bank_account_id:  editTx.sourceAccountId ?? '',
        work_unit_id:     editTx.unitId ?? '',
      })
    } else {
      setForm({ ...emptyForm(), work_unit_id: lockedUnitId ?? '' })
    }
    setFieldErrors({})
  }, [open, editTx, lockedUnitId])

  const kategoriList = getKategoriList(form.jenis)

  function set(field: keyof FormState, value: string) {
    setForm(prev => {
      const next = { ...prev, [field]: value }
      // Reset kategori jika jenis berubah
      if (field === 'jenis') next.kode_rekening = ''
      return next
    })
    setFieldErrors(prev => ({ ...prev, [field]: undefined }))
  }

  function validate(): boolean {
    const errs: Partial<Record<keyof FormState, string>> = {}
    if (!form.transaction_date)              errs.transaction_date = 'Tanggal wajib diisi'
    if (!form.description.trim())            errs.description      = 'Uraian wajib diisi'
    if (!form.jenis)                         errs.jenis            = 'Jenis pengeluaran wajib dipilih'
    if (!form.kode_rekening)                 errs.kode_rekening    = 'Kategori wajib dipilih'
    if (parseRupiah(form.amountFormatted) <= 0) errs.amountFormatted = 'Nominal harus lebih dari 0'
    if (!form.bank_account_id)               errs.bank_account_id  = 'Rekening bank wajib dipilih'
    setFieldErrors(errs)
    return Object.keys(errs).length === 0
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!validate()) return

    const payload = {
      type:             'OUT' as const,
      transaction_date: form.transaction_date,
      no_bukti:         form.no_bukti,
      description:      form.description,
      kode_rekening:    form.kode_rekening,
      amount:           parseRupiah(form.amountFormatted),
      work_unit_id:     form.work_unit_id || null,
      period_id:        activePeriod?.id ?? null,
      source_account_id: form.bank_account_id || null,
      jenis_pendapatan_id:  null,
      sumber_pendapatan_id: null,
    }

    const ok = isEdit && editTx
      ? await updateTransaction(editTx.id, payload)
      : await insertTransaction(payload)
    if (ok) { onSuccess(); onClose() }
  }

  const title = isEdit ? 'Edit Pengeluaran BPN' : 'Catat Pengeluaran BPN'

  return (
    <Modal open={open} onClose={onClose} title={title} maxWidth="md">
      <form onSubmit={handleSubmit} noValidate style={{ colorScheme: 'dark' }}>
        <div className="px-6 py-5 space-y-4">

          {/* Row: Tanggal + Nomor Bukti */}
          <div className="grid grid-cols-2 gap-4">
            <Field label="Tanggal" error={fieldErrors.transaction_date} required>
              <input
                type="date"
                value={form.transaction_date}
                onChange={e => set('transaction_date', e.target.value)}
                className={inputCls(!!fieldErrors.transaction_date)}
              />
            </Field>
            <Field label="Nomor Bukti" hint="Kosongkan untuk auto-generate">
              <input
                type="text"
                placeholder="BPN-K-001"
                value={form.no_bukti}
                onChange={e => set('no_bukti', e.target.value)}
                className={inputCls(false)}
              />
            </Field>
          </div>

          {/* Uraian */}
          <Field label="Uraian / Deskripsi" error={fieldErrors.description} required>
            <textarea
              rows={2}
              placeholder="Contoh: Pengembalian UKT mahasiswa atas nama ..."
              value={form.description}
              onChange={e => set('description', e.target.value)}
              className={inputCls(!!fieldErrors.description) + ' resize-none'}
            />
          </Field>

          {/* Jenis Pengeluaran */}
          <Field label="Jenis Pengeluaran" error={fieldErrors.jenis} required>
            <div className="flex gap-3">
              {JENIS_OPTIONS.map(opt => (
                <button
                  key={opt.value}
                  type="button"
                  onClick={() => set('jenis', opt.value)}
                  className={[
                    'flex-1 py-2.5 px-3 rounded-xl text-sm font-medium font-body border transition-colors',
                    form.jenis === opt.value
                      ? 'bg-primary/20 border-primary text-primary'
                      : 'bg-[#1e2430] border-[#2a303c] text-[#e8eaf0] hover:border-primary/50',
                  ].join(' ')}
                >
                  {opt.label}
                </button>
              ))}
            </div>
            {fieldErrors.jenis && (
              <span className="text-xs text-error">{fieldErrors.jenis}</span>
            )}
          </Field>

          {/* Row: Kategori + Nominal */}
          <div className="grid grid-cols-2 gap-4">
            <Field label="Kategori Pengeluaran" error={fieldErrors.kode_rekening} required>
              <select
                value={form.kode_rekening}
                onChange={e => set('kode_rekening', e.target.value)}
                disabled={!form.jenis}
                className={inputCls(!!fieldErrors.kode_rekening) + (!form.jenis ? ' opacity-50 cursor-not-allowed' : '')}
              >
                <option value="">— Pilih Kategori —</option>
                {kategoriList.map(k => (
                  <option key={k} value={k}>{k}</option>
                ))}
              </select>
            </Field>

            <Field label="Nominal (Rp)" error={fieldErrors.amountFormatted} required>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#bfc8c4] text-sm select-none">Rp</span>
                <input
                  type="text"
                  inputMode="numeric"
                  placeholder="0"
                  value={form.amountFormatted}
                  onChange={e => set('amountFormatted', formatRupiah(e.target.value))}
                  className={inputCls(!!fieldErrors.amountFormatted) + ' pl-9 text-right'}
                />
              </div>
            </Field>
          </div>

          {/* Rekening Bank */}
          <Field label="Rekening Bank Penerimaan" error={fieldErrors.bank_account_id} required>
            <select
              value={form.bank_account_id}
              onChange={e => set('bank_account_id', e.target.value)}
              className={inputCls(!!fieldErrors.bank_account_id)}
            >
              <option value="">— Pilih Rekening Bank —</option>
              {bankAccounts.map(a => (
                <option key={a.id} value={a.id}>
                  {a.bank_name} — {a.account_number} a.n. {a.account_name}
                </option>
              ))}
            </select>
          </Field>

          {/* Unit Kerja */}
          <Field
            label="Unit Kerja"
            hint={lockedUnitId ? 'Unit kerja sesuai akun Anda' : 'Opsional — kosongkan untuk transaksi pusat'}
          >
            <select
              value={form.work_unit_id}
              onChange={e => set('work_unit_id', e.target.value)}
              disabled={!!lockedUnitId}
              className={inputCls(false) + (lockedUnitId ? ' opacity-70 cursor-not-allowed' : '')}
            >
              {!lockedUnitId && <option value="">— Pusat (tanpa unit kerja) —</option>}
              {workUnits.map(u => (
                <option key={u.id} value={u.id}>{u.name}</option>
              ))}
            </select>
          </Field>

          {/* Periode aktif (read-only info) */}
          {activePeriod && (
            <div className="text-xs text-[#bfc8c4] bg-[#1e2430] border border-[#2a303c] rounded-lg px-3 py-2">
              Periode aktif:{' '}
              <span className="font-semibold text-[#e8eaf0]">
                {activePeriod.code ?? `${activePeriod.month}/${activePeriod.year}`}
              </span>
            </div>
          )}

          {/* Error dari server */}
          {error && (
            <div className="text-sm text-error bg-error-container rounded-lg px-3 py-2">{error}</div>
          )}
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10">
          <Button type="button" variant="secondary" size="sm" onClick={onClose} disabled={saving}>
            Batal
          </Button>
          <Button type="submit" variant="primary" size="sm" disabled={saving} icon={saving ? undefined : 'save'}>
            {saving ? 'Menyimpan…' : isEdit ? 'Simpan Perubahan' : 'Simpan sebagai Draft'}
          </Button>
        </div>
      </form>
    </Modal>
  )
}

// ── Sub-components ─────────────────────────────────────────────────────────────

function Field({
  label, children, error, hint, required,
}: {
  label: string; children: React.ReactNode; error?: string; hint?: string; required?: boolean
}) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-medium text-[#bfc8c4]">
        {label}{required && <span className="text-error ml-0.5">*</span>}
      </label>
      {children}
      {error && <span className="text-xs text-error">{error}</span>}
      {!error && hint && <span className="text-xs text-[#bfc8c4]/70">{hint}</span>}
    </div>
  )
}

function inputCls(hasError: boolean) {
  return [
    'w-full px-3 py-2 rounded-xl text-sm bg-[#1e2430]',
    'border text-[#e8eaf0]',
    hasError
      ? 'border-error focus:ring-error/30 focus:border-error'
      : 'border-[#2a303c] focus:border-primary',
    'focus:outline-none focus:ring-2 focus:ring-primary/40',
    'transition-colors placeholder:text-white/30',
  ].join(' ')
}
