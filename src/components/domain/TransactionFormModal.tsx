...
import type { KategoriPengeluaran } from '../../types'
...
export function TransactionFormModal({
  ...
  const { categories: revenueCategories } = useRevenueCategories()
  const { businessUnits } = useBusinessUnits()
  const { fundingSources } = useFundingSources()
  const { items: sumberPendapatan } = useSumberPendapatanBisnis()
  ...
  useEffect(() => {
    ...
    if (editTx) {
      setForm({
        ...
        bank_account_id:     (txType === 'IN' ? editTx.destinationAccountId : editTx.sourceAccountId) ?? '',
        jenis_pendapatan_id:  '',
        sumber_pendapatan_id: '',
      })
    } else {
  ...
  const mekanismeBisnis = (() => {
    if (!isBPN || !form.kode_rekening) return ''
    const cat = revenueCategories.find(c => c.name === form.kode_rekening)
    if (!cat?.business_unit_id) return ''
    return businessUnits.find(b => b.id === cat.business_unit_id)?.name ?? ''
  })()
  ...
  function validate(): boolean {
    ...
    if (txType === 'IN' && !form.jenis_pendapatan_id) errs.jenis_pendapatan_id = 'Jenis pendapatan wajib dipilih'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      jenis_pendapatan_id:  form.jenis_pendapatan_id  || null,
      sumber_pendapatan_id: form.sumber_pendapatan_id || null,
    }
    ...
    <Modal open={open} onClose={onClose} title={title} maxWidth="md">
      <form onSubmit={handleSubmit} noValidate>
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Kategori Pendapatan" error={fieldErrors.kode_rekening} required>
              <select
          ...
          {/* Mekanisme Bisnis — hanya BPN, read-only, auto-fill dari kategori */}
          {isBPN && (
            <Field label="Mekanisme Bisnis" hint="Otomatis terisi berdasarkan kategori yang dipilih">
              <div className="relative">
                <input
                  type="text"
                  value={mekanismeBisnis || '— Belum terhubung ke mekanisme bisnis —'}
                  readOnly
                  disabled
                  className={[
                    inputCls(false),
          ...
          {/* Jenis Pendapatan — hanya BPN */}
          {isBPN && (
            <Field label="Jenis Pendapatan" error={fieldErrors.jenis_pendapatan_id} required>
              <select
                value={form.jenis_pendapatan_id}
                onChange={e => set('jenis_pendapatan_id', e.target.value)}
                className={inputCls(!!fieldErrors.jenis_pendapatan_id)}
              >
          ...
          {/* Sumber Pendapatan Bisnis — hanya BPN */}
          {isBPN && (
            <Field label="Sumber Pendapatan Bisnis" hint="Opsional — sumber pendapatan bisnis terkait">
              <select
                value={form.sumber_pendapatan_id}
                onChange={e => set('sumber_pendapatan_id', e.target.value)}
                className={inputCls(false)}
              >
                <option value="">— Pilih Sumber Pendapatan Bisnis —</option>
                {sumberPendapatan.map(s => (
                  <option key={s.id} value={s.id}>
                    {s.kode ? `[${s.kode}] ` : ''}{s.name}
                  </option>
                ))}
              </select>
            </Field>
          ...
          <Field
            label="Unit Kerja"
            hint={lockedUnitId
              ? 'Unit kerja sesuai akun Anda'
              : 'Opsional — kosongkan untuk transaksi pusat'}
          >
            <select
